...
// Two independent hoistable transfers in one stage emit two separate
// sibling pipelines (no grouping). Each transfer's hoist target is the
// same loop (%n), so both end up as adjacent siblings in the m-loop body.

// CHECK-LABEL:   func.func @two_hoists
...
// CHECK-NEXT:       ktdf.pipeline {
// CHECK-NEXT:         ktdf.stage depends_in(none) depends_out(none) {
// CHECK-NEXT:           ktdf.data_transfer from %[[B]][%[[IM]]] size [64] to %[[NEW_L1_B]][%[[C0]]] size [64] : memref<?xf16, "DDR">, memref<64xf16, "L1">
// CHECK-NEXT:         }
// CHECK-NEXT:       }
...
module {
  func.func @two_hoists(%A: memref<?xf16, "DDR">,
                        %B: memref<?xf16, "DDR">,
                        %M: index, %N: index) {
      ...
      scf.for %n = %c0 to %N step %c1 {
        ktdf.pipeline {
          ...
          ktdf.stage depends_in(none) depends_out(%t) {
            ...
            ktdf.data_transfer from %B[%m] size [64] to %l1_B[%c0] size [64]
              : memref<?xf16, "DDR">, memref<64xf16, "L1">
          }
