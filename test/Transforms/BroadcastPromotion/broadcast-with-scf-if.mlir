...
// An invariant transfer gated by an scf.if hoists with the scf.if
// preserved around it. Note: the donor stage is left holding an empty
// scf.if (the original scf.if minus its body content); RegionClonePrune
// preserves structural skeletons in the donor as well as in the clone.

// CHECK-LABEL:   func.func @scf_if_hoist
// CHECK-SAME:        %[[A:[^,]+]]: memref<?xf16, "DDR">, %[[M:[^,]+]]: index, %[[N:[^,]+]]: index, %[[COND:[^,]+]]: i1) {
...
module {
  func.func @scf_if_hoist(%A: memref<?xf16, "DDR">,
                          %M: index, %N: index, %cond: i1) {
