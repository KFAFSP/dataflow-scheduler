// Path expansion of an unrealizable FIFO type.
//
// The pipeline requests a !ktdf.fifo.slot<"DDR" -> "SFU">, which the device
// does not implement: the only FIFO reaching the SFU is the L1LU one fed from
// L1.  Path expansion has to route around it, turning the single transfer into
// a DDR -> L1 DMA (MNILU) into a private L1 buffer followed by the L1 -> SFU
// FIFO (L1LU).  The store side already uses the realizable
// !ktdf.fifo.slot<"SFU" -> "L1SU">, so it must come out unchanged.
